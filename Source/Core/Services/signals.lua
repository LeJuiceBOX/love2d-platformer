...
function SignalConnection:initialize(signalSystem,signal,connectionHash,callback)
    assert(typeof(signalSystem) == 'SignalSystem', "An instance of 'SignalSystem' must be the first parameter. (got '"..typeof(signalSystem).."')")
...
local SignalSystem = Class("SignalSystem")

function SignalSystem:initialize()
    self._signals = {}
end

function SignalSystem:print()
    for i,v in pairs(self._signals) do
        print(i,v)
    end
end

function SignalSystem:newSignal(signalName)
...
function SignalSystem:getSignal(signalName)
    assert(self._signals[signalName] ~= nil,"Signal doesnt exist. ('"..tostring(signalName).."')")
    return self._signals[signalName]
end


function SignalSystem:uuid(randomFunction)
...
function SignalSystem:newHash(randomFunction)
